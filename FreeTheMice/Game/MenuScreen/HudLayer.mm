...
-(id) init
{
	if( (self=[super init])) {
        ...
        [cheezeCollectedBg setScale:0.5];
...
-(void) updateTimeRemaining:(int)minuts andTimeInSec:(int)seconds{
    ...
    float totalTime = 120;
    [timer setScaleX:(time/totalTime) * 0.5];
...
//        
//    }];
//    [moveRightMenuItem setScale:0.5];
...
-(void) addPauseScreen {
    ...
    CCMenu *pauseMenu = [CCMenu menuWithItems:homeMenuItem,soundMenuItem,levelsMenuItem, nil];
...
-(void) showPausingAnimation{
    ...
    if(pauseScreenBg.tag ==125){
        [self hideHudItems];
        CCScaleTo *scaleAction = [CCScaleTo actionWithDuration:0.03 scaleX:0.5 scaleY:0.5];
